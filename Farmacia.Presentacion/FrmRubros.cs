...
namespace Farmacia.Presentacion
{
    public partial class FrmRubros : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgprincipal.Columns[1].HeaderText = "RUBROS";
        }
        private void Listado(string texto)
        {
            try
            {
                Dgprincipal.DataSource = NRubro.Listado(texto);
                this.Formato();
            }
            catch (Exception ex)
            {
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(Dgprincipal.CurrentRow.Cells["id_ru"].Value)))
            {
            ...
            else
            {
                ...
                Txtdescripcion.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["descripcion_ru"].Value);
            }
        }
        ...
        private void Btnguardar_Click(object sender, EventArgs e)
        {
            ...
            else //Se procedera a insertar
            {
                ...
                obj.Descripcion_ru = Txtdescripcion.Text.Trim();
                Rpta = NRubro.Guardar(opcion, obj);
                if (Rpta == "OK")
                {
                    this.Listado("%");
        ...
        private void Btneliminar_Click(object sender, EventArgs e)
        {
            ...
            else //Se procedera a eliminar
            {
                ...
                if (opc == DialogResult.Yes)
                {
                    string Rpta = "";
                    this.id_ru = Convert.ToInt32(Dgprincipal.CurrentRow.Cells["id_ru"].Value);
                    Rpta = NRubro.Eliminar(this.id_ru);
                    if (Rpta.Equals("OK"))
                    {
                        this.Listado("%");
                        this.id_ru = 0;
        ...
        {
           Reportes.FrmRpt_Rubros obj = new Reportes.FrmRpt_Rubros();
