...
namespace Farmacia.Presentacion
{
    public partial class FrmClientes : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgprincipal.Columns[0].HeaderText = "CÓDIGO_CL";
        ...
        private void Listado(string texto)
        {
            try
            {
                Dgprincipal.DataSource = NCliente.Listado(texto);
                this.Formato();
            }
            catch (Exception ex)
            {
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(Dgprincipal.CurrentRow.Cells["id_cl"].Value)))
            {
            ...
            else
            {
                ...
                Txtnrodocumento_cl.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["nrodocumento_cl"].Value);
                Txtrazon_social_cl.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["razon_social_cl"].Value);
                ...
                Txtemail.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["email_cl"].Value);
                Txttelefono.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["telefono_cl"].Value);
                Txtmovil.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["movil_cl"].Value);
                ...
                Txtdireccion_cl.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["direccion_cl"].Value);
                ...
                Txtobservacion.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["observacion_cl"].Value).Trim();
            }
        }
        ...
        private void Listado_tdpc()
        {
            try
            {
                Dgv_tipo_tdpc.DataSource = NCliente.Listado_tdpc();
                this.Formato_tdpc();
            }
            catch (Exception ex)
            {
        ...
        private void Listado_sx()
        {
            try
            {
                Dgv_sexos.DataSource = NCliente.Listado_sx();
                this.Formato_sx();
            }
            catch (Exception ex)
            {
        ...
        private void Listado_ru(string cTexto)
        {
            try
            {
                Dgv_rubros.DataSource = NCliente.Listado_ru(cTexto);
                this.Formato_ru();
            }
            catch (Exception ex)
            {
        ...
        private void Listado_di(string cTexto)
        {
            try
            {
                Dgv_distritos.DataSource = NCliente.Listado_di_cl(cTexto);
                this.Formato_di();
            }
            catch (Exception ex)
            {
        ...
        {
            Txtnrodocumento_cl.ReadOnly = !estado;
            Txtrazon_social_cl.ReadOnly = !estado;
            ...
            Txtdireccion_cl.ReadOnly = !estado;
            Txtobservacion.ReadOnly = !estado;
        }

        private void Limpia_texto()
        {
            Txtnrodocumento_cl.Text = "";
            Txtrazon_social_cl.Text = "";
            Txtnombres.Text = "";
            Txtapellidos.Text = "";
            Txtemail.Text = "";
            Txttelefono.Text = "";
            Txtmovil.Text = "";
            Txtdireccion_cl.Text = "";
            Txtobservacion.Text = "";
        }
        ...
        {
            if  (Txtnrodocumento_cl.Text == String.Empty ||
                ...
                Txtdireccion_cl.Text == String.Empty)
            ...
            else // Se procedera a insertar
            {
                ...
                obj.Email_cl = Txtemail.Text.Trim();
                obj.Telefono_cl = Txttelefono.Text.Trim();
                obj.Movil_cl = Txtmovil.Text.Trim();
                obj.Direccion_cl = Txtdireccion_cl.Text.Trim();
                obj.Id_di = this.id_di;
                obj.Observacion_cl = Txtobservacion.Text.Trim();



                Rpta = NCliente.Guardar(opcion, obj);
                  if (Rpta == "OK")
                {
                    this.Listado("%");
        ...
        private void Btneliminar_Click(object sender, EventArgs e)
        {
            ...
            else //Se procedera a eliminar
            {
                ...
                if (opc == DialogResult.Yes)
                {
                    string Rpta = "";
                    this.id_cl = Convert.ToInt32(Dgprincipal.CurrentRow.Cells["id_cl"].Value);
                    Rpta = NCliente.Eliminar(this.id_cl);
                    if (Rpta.Equals("OK"))
                    {
                        this.Listado("%");
                        this.id_cl = 0;
        ...
        {
            Reportes.FrmRpt_Clientes obj = new Reportes.FrmRpt_Clientes();
