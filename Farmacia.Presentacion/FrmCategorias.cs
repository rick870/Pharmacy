...
namespace Farmacia.Presentacion
{
    public partial class FrmCategorias : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgprincipal.Columns[1].HeaderText = "CATEGORÍA";
        }
        private void Listado(string texto)
        {
            try
            {
                Dgprincipal.DataSource = NCategoria.Listado(texto);
                this.Formato();
            }
            catch (Exception ex)
            {
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(Dgprincipal.CurrentRow.Cells["id_ca"].Value)))
            {
            ...
            else
            {
                ...
                Txtdescripcion.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["descripcion_ca"].Value);
            }
        }
        ...
        private void Btnguardar_Click(object sender, EventArgs e)
        {
            ...
            else //Se procedera a insertar
            {
                ...
                obj.Descripcion_ca = Txtdescripcion.Text.Trim();
                Rpta = NCategoria.Guardar(opcion, obj);
                if (Rpta == "OK")
                {
                    this.Listado("%");
        ...
        private void Btneliminar_Click(object sender, EventArgs e)
        {
            ...
            else //Se procedera a eliminar
            {
                ...
                if (opc == DialogResult.Yes)
                {
                    string Rpta = "";
                    this.id_ca = Convert.ToInt32(Dgprincipal.CurrentRow.Cells["id_ca"].Value);
                    Rpta = NCategoria.Eliminar(this.id_ca);
                    if (Rpta.Equals("OK"))
                    {
                        this.Listado("%");
                        this.id_ca = 0;
        ...
        {
           Reportes.FrmRpt_Categorias obj = new Reportes.FrmRpt_Categorias();
