...
namespace Farmacia.Presentacion
{
    public partial class FrmAlmacenes : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgprincipal.Columns[1].HeaderText = "ALMACEN";
        }
        private void Listado(string texto)
        {
            try
            {
                Dgprincipal.DataSource = NAlmacen.Listado(texto);
                this.Formato();
            }
            catch (Exception ex)
            {
        ...
        private void Selecciona_item()
        {
            ...
            else
            {
                ...
                Txtdescripcion.Text = Convert.ToString(Dgprincipal.CurrentRow.Cells["descripcion_al"].Value);
            }
        }
        ...
        private void Btnguardar_Click(object sender, EventArgs e)
        {
            ...
            else //Se procedera a insertar
            {
                ...
                obj.Descripcion_al = Txtdescripcion.Text.Trim();
                Rpta = NAlmacen.Guardar(opcion, obj);
                if (Rpta == "OK")
                {
                    this.Listado("%");
        ...
        private void Btneliminar_Click(object sender, EventArgs e)
        {
            ...
            else //Se procedera a eliminar
            {
                ...
                if (opc == DialogResult.Yes)
                {
                    string Rpta = "";
                    this.id_al = Convert.ToInt32(Dgprincipal.CurrentRow.Cells["id_al"].Value);
                    Rpta = NAlmacen.Eliminar(this.id_al);
                    if (Rpta.Equals("OK"))
                    {
                        this.Listado("%");
                        this.id_al = 0;
        ...
        {
           Reportes.FrmRpt_Almacenes obj = new Reportes.FrmRpt_Almacenes();
